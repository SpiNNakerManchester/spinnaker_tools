  ...
  if (sdp_msg->cmd_rc == 4) // 1. it's a trace message from the host
    {
      ...
      nodes = (int***) sark_xalloc(sv->sdram_heap, xChips * sizeof(int**),
	      0, ALLOC_LOCK);

      for (int i = 0; i < xChips; i++)
        {
	  nodes[i] = (int**) sark_xalloc(sv->sdram_heap, yChips * sizeof(int*),
		  0, ALLOC_LOCK);
	  for (int j = 0; j < yChips; j++)
	    {
	      nodes[i][j] = (int*) sark_xalloc(sv->sdram_heap, cores * sizeof(int),
		      0, ALLOC_LOCK);
	    }
        }
